...
# Запитуємо в користувача перше число
while True:
    string_input = input('Input your number one: ')
...
# Запитуємо в користувача друге число
while True:
    string_input = input('Input your number two: ')
    true, number_type = check_number(string_input)
    if true:
        if number_type == 'int':
            number_2 = int(string_input)
        else:
            number_2 = float(string_input)
        break
    else:
        print('   The data entered is not a number...')
